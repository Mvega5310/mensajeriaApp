...
> El aislamiento ya lo aporta la extensión (Fase B). El trabajo de la Fase E es **demostrarlo con
...
- [ ] **E3. Integración HTTP — aislamiento de `schedule`/`checkin`/`confirm-delivery` y sin `fotoUrl`.**
  ...
  - **Nota:** `getFoto`/`schedule` usan `findUnique` con `conjuntoId` en el **primer nivel**
...
- [ ] **E7. Auditorías manuales → pruebas estáticas.**
  - Recorre `backend/src` y `backend/prisma` y falla si: `GLOBAL_LOOKUP` se abre fuera de
    ...
    carpetas de prueba solo donde corresponda, documentado en la prueba.
  - _Requisitos:_ §2.3.1, §2.3, §6.1
